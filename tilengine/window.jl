...
const tln_createWindow_ptr = Libdl.dlsym(tlnLib, "TLN_CreateWindow")
...
const tln_processWindow_ptr = Libdl.dlsym(tlnLib, "TLN_ProcessWindow")
function processWindow()::Bool
    return ccall(tln_processWindow_ptr, Bool, ())
end

const tln_drawFrame_ptr = Libdl.dlsym(tlnLib, "TLN_DrawFrame")
function drawFrame(frame::Integer)::Nothing
    ccall(tln_drawFrame_ptr, Nothing, (Cint,), Cint(frame))
    return
end

const tln_setWindowTitle_ptr = Libdl.dlsym(tlnLib, "TLN_SetWindowTitle")
